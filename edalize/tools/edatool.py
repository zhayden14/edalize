...
class Edatool(object):
    ...
    def configure(self, edam):
        ...
        # FIXME: Properly merge with tool options set by flow
        self.tool_options.update(edam.get("tool_options", {}).get(_tool_name, {}))
    ...
    def _apply_parameters(self, args):
        for key, value in args.items():
            ...
            if value is None:
                pass
            # If the parameter is a tool option we handle it here
            elif key in self.TOOL_OPTIONS:
                if self.TOOL_OPTIONS[key].get("list"):
